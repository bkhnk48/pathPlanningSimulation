...
class GraphProcessor:
    ...
    def write_to_file(self, Max, filename = "TSG.txt"):
        adjust = 1 if config.solver_choice != 'solve' else 0
        with open('TSG.txt', 'w') as file:
            ...
            for edge in self.ts_edges:
                ...
                if edge is not None:   
                    if(edge.weight == self.H*self.H):
                        #pdb.set_trace()
                        file.write(f"c Exceed {edge.weight + adjust} {edge.weight // self.M}\na {edge.start_node.id} {edge.end_node.id} {edge.lower} {edge.upper} {edge.weight + adjust}\n")
                    else:
                        file.write(f"a {edge.start_node.id} {edge.end_node.id} {edge.lower} {edge.upper} {edge.weight + adjust}\n")
        if(self.printOut):
